...
# ...but imagine list_data argument is not even a list - it is just ONE data frame
result2 <- subsetTopNAndJoinDataTOresult(list_data = df1, data2join = df_hp, key = "hp")
```

The problem is that both `data.frame` and `list`:
...
Suggest validation of `unitTestsWorkshopPackage::subsetTopNAndJoinDataTOresult` argument

## Task 2

Handle and test exceptions when:

* `list_data` and `data2join` have 0 rows
* 
