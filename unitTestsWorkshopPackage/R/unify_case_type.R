...
unify_case_type <- function(DT_frame,  cols2lower, cols2upper ) {
  ...
  if (!is.null(cols2lower) && length(cols2lower) > 0 ) {
    ...
    futile.logger::flog.info(sprintf("Converting columns %s to lower case", 
                                     paste0(cols2lower, collapse = ", ")), 
                             name = "unify_case_type")
  ...
  if (!is.null(cols2upper) && length(cols2upper) > 0 ) {  
    ...
    futile.logger::flog.info(sprintf("Converting columns %s to upper case", 
                                     paste0(cols2upper, collapse = ", ")), 
                             name = "unify_case_type")
